    ...
    _.extend(WipePhoneDialog.prototype, {
        ...
        _hideDialog: function () {
            ...
            if(!this.$el.find(".step1").hasClass("current")) {
                ...
                this.$el.find(".step1").addClass("current");
            }
            this.hide();
        },
        ...
        processDialogView: function (step) {
            ...
            if (step) {
                this.$el.find(".wiz_step").removeClass("current");
                this.$el.find("." + step + ".wiz_step").addClass("current");
            }

            if (this.$el.find(".step1").hasClass("current")) {
                // Hide all buttons except "Cancel"
                this.$el.find(".step1 .dialog_buttons ul li:not(.btn_cancel)").hide();

                // If lock is disabled, remove lock terminology and buttons
                if (!AMA.config.lockEnabled) {
                    this.$el.find(".step1 .no_lock").hide();
                }
                ...
                if (adminEnabled) {
                    this.$el.find(".step1 .dialog_buttons .btn_next").show();
                } else {
                    if (AMA.config.lockEnabled) {
                        this.$el.find(".step1 .dialog_buttons .btn_erase_and_lock").show();
                    } else {
                        this.$el.find(".step1 .dialog_buttons .btn_erase").show();
                    }

                    if (AMA.models.capabilities.canCreate("syncEvents")) {
                        this.$el.find(".step1 .dialog_buttons .nosync:visible").hide();
                    } else {
                        this.$el.find(".step1 .dialog_buttons .sync:visible").hide();
                    }
                }
            } else if(this.$el.find(".step2").hasClass("current")) {
                this.$el.find(".step2 .warning").hide();
                this.$el.find(".step2 .dialog_buttons ul li:not(.btn_cancel)").hide();
                if (this.$el.find("#wipephone_wipe:checked").length > 0) {
                    if (syncAndOrWipeSupported) {
                        this.$el.find(".step2 .warning.optionalsync").show();
                        this.$el.find(".step2 .confirm .dialog_buttons .btn_sync_then_wipe").show();
                        this.$el.find(".step2 .confirm .dialog_buttons .btn_nosync_then_wipe").show();
                        ...
                        if (!AMA.config.lockEnabled) {
                            this.$el.find(".step2 .confirm .dialog_buttons .btn_erase_and_lock").hide();
                            this.$el.find(".step2 .confirm .dialog_buttons .btn_erase").show();
                        } else {
                            this.$el.find(".step2 .confirm .dialog_buttons .btn_erase_and_lock").show();
                            this.$el.find(".step2 .confirm .dialog_buttons .btn_erase").hide();
                        }
                        
                        if (AMA.models.capabilities.canCreate("syncEvents")) {
                            this.$el.find(".step2 .confirm .dialog_buttons .nosync:visible").hide();
                        } else {
                            this.$el.find(".step2 .confirm .dialog_buttons .sync:visible").hide();
                        }
                    }
                } else {
                    this.$el.find(".title").addClass("hidden");
                    ...
                    if (syncAndOrWipeSupported) {
                        this.$el.find(".step2 .warning.optionalsync").show();
                        this.$el.find(".step2 .confirm .dialog_buttons .btn_sync_then_reset").show();
                        this.$el.find(".step2 .confirm .dialog_buttons .btn_nosync_then_reset").show();
                        ...
                        this.$el.find(".step2 .confirm .dialog_buttons .btn_factory_reset_device").show();
        ...
        _factorySuccessLogout: function () {
        	...
        	location = "index.html#factory_reset";
        }
    });
})();
