...
(function () {
	...
	AMA.augment(LocationHistoryListView.prototype, {
		...
		events: {
            ...
            "click  #button_locate_normal": "locate"
        },
		...
		initialize: function () {
			...
			this.bind(LocationHistoryListView.EVENT.ADDRESS_RETRIEVED, function(model) {
				o.$el.find("div[uid=" + model.get("id") + "] .address").html(model.get("address"));
			});
		...
		render: function () {
			...
			if (this.data.length > 0) {
				...
				for (var i = 0; i < this.data.numberOfPoints; i++) {
					...
					if (location) {
						content += _.template(this.template, this._processData(location, i));
						if (i == 0) {
