...
require([
    ...
    var AppRouter = Backbone.Router.extend({
        routes: {
            ...
            'password_retrieve_secure?*queryString' : 'getSecurityQuestion'
        ...
        factory_reset: function() {
        	$('#helpWrapper').hide();
            $('#wrapper').show();
            require(["view/prelogin/FactoryResetView"], function(FactoryResetView) {
                var factoryResetView = new FactoryResetView().render();
            });
        },
        learnmore: function() {
        ...
        login_failed: function() {
            require(["view/prelogin/IndexView", "view/prelogin/LoginFailedView"], function(IndexView, LoginFailedView) {
                var indexView = new IndexView().render();
                var loginFailedView = new LoginFailedView().render();
            });
        },
        login_invalid: function() {
            require(["view/prelogin/IndexView", "view/prelogin/LoginInvalidView"], function(IndexView, LoginInvalidView) {
                var indexView = new IndexView().render();
        ...
        password_retrieve_secure_wrong: function(queryString) {
        ...
        getSecurityQuestion: function(queryString) {
            require(["view/prelogin/PasswordRetrieveSecureView"], function(PasswordRetrieveSecureView) {
				var params = parseQueryString(queryString);

				var secQuestionUrl = AMA.config.apiHostUrl + "/accounts/passwordReset/confirm?" + $.param({ devId: AMA.config.devId, uniqueToken: params.uniqueToken });
				
				$.get(secQuestionUrl, null, function(response) {
					response["uniqueToken"] = params.uniqueToken;
					
					var passwordRetrieveSecureView = new PasswordRetrieveSecureView(response).render();
				});
